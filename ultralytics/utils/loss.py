...
class BboxLoss(nn.Module):
    ...
    def forward(
        ...
        if metrics.USE_NWD:  # YOLO-SEA tiny-object blend; boxes are in grid units -> rescale to pixels for NWD_C
            s = stride.view(1, -1).expand_as(fg_mask)[fg_mask].unsqueeze(-1)
            nwd = wasserstein_similarity(pred_bboxes[fg_mask] * s, target_bboxes[fg_mask] * s)
            iou = (1 - metrics.NWD_RATIO) * iou + metrics.NWD_RATIO * nwd
