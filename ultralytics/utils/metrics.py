...
# YOLO-SEA tiny-object option — blend Normalized Gaussian Wasserstein Distance (NWD) into the
# box-similarity used by the training loss and the task-aligned assigner. IoU is near-binary for
# boxes a few pixels wide (a 1 px shift can zero it out); NWD degrades smoothly with center/size
# deviation, which is what makes tiny distant ships learnable (https://arxiv.org/abs/2110.13389).
# Off by default — no effect on stock models. Enable at training time:
#   import ultralytics.utils.metrics as metrics
#   metrics.USE_NWD = True       # turn the IoU/NWD blend on globally
#   metrics.NWD_RATIO = 0.5      # 0 = pure CIoU, 1 = pure NWD
#   metrics.NWD_C = 12.8         # normalization constant in *pixels* (~mean absolute target size)
USE_NWD = False
NWD_RATIO = 0.5
NWD_C = 12.8
...
def wasserstein_similarity(
    box1: torch.Tensor, box2: torch.Tensor, constant: float | None = None, eps: float = 1e-7
) -> torch.Tensor:
    """Normalized Gaussian Wasserstein Distance similarity between bounding boxes.

    Boxes are modeled as 2D Gaussians N(center, diag(w/2, h/2)^2); the similarity is
    ``exp(-sqrt(W2^2) / C)`` where W2 is the 2nd-order Wasserstein distance between the Gaussians.
    Returns values in (0, 1], shaped like ``bbox_iou`` output (last dim kept).

    Args:
        box1 (torch.Tensor): Boxes in (x1, y1, x2, y2) format, last dimension 4, in **pixel** units.
        box2 (torch.Tensor): Boxes in (x1, y1, x2, y2) format, broadcastable with box1.
        constant (float, optional): Normalization constant C in pixels. Defaults to module ``NWD_C``.
        eps (float): Small value for numerical stability of the sqrt.

    Returns:
        (torch.Tensor): NWD similarity with the same leading shape as the inputs, last dim 1.
    """
    c = NWD_C if constant is None else constant
    b1_x1, b1_y1, b1_x2, b1_y2 = box1.chunk(4, -1)
    b2_x1, b2_y1, b2_x2, b2_y2 = box2.chunk(4, -1)
    center_dist2 = ((b1_x1 + b1_x2) - (b2_x1 + b2_x2)).pow(2) / 4 + ((b1_y1 + b1_y2) - (b2_y1 + b2_y2)).pow(2) / 4
    wh_dist2 = (((b1_x2 - b1_x1) - (b2_x2 - b2_x1)).pow(2) + ((b1_y2 - b1_y1) - (b2_y2 - b2_y1)).pow(2)) / 4
    return torch.exp(-torch.sqrt(center_dist2 + wh_dist2 + eps) / c)
