...
# --- YOLO-SEA Soft-NMS configuration -------------------------------------------------------------
# Soft-NMS (https://arxiv.org/abs/1704.04503) decays the confidence of overlapping boxes instead of
# hard-suppressing them. It requires NO retraining and can be toggled on any checkpoint. Enable with:
#   import ultralytics.utils.nms as nms
#   nms.USE_SOFT_NMS = True            # turn Soft-NMS on globally
#   nms.SOFT_NMS_SIGMA = 0.5           # gaussian decay sigma
#   nms.SOFT_NMS_METHOD = "gaussian"   # or "linear"
USE_SOFT_NMS = False
SOFT_NMS_SIGMA = 0.5
SOFT_NMS_METHOD = "gaussian"  # {"gaussian", "linear"}
...
def non_max_suppression(
    ...
    soft = USE_SOFT_NMS if soft is None else soft  # YOLO-SEA: resolve Soft-NMS toggle from global flag
    soft_sigma = SOFT_NMS_SIGMA if soft_sigma is None else soft_sigma
    ...
    for xi, (x, xk) in enumerate(zip(prediction, xinds)):  # image index, (preds, preds indices)
        ...
        elif soft:  # YOLO-SEA Soft-NMS: decay confidences of overlapping boxes (no hard suppression)
            boxes = x[:, :4] + c  # boxes (offset by class)
            i, soft_scores = TorchNMS.soft_nms(
                boxes, scores, iou_thres, sigma=soft_sigma, method=SOFT_NMS_METHOD, score_thres=conf_thres
            )
            x[:, 4] = soft_scores  # write back decayed confidences
        else:
            boxes = x[:, :4] + c  # boxes (offset by class)
...
class TorchNMS:
    ...
    def soft_nms(
        ...
        Instead of removing every box whose IoU with a kept box exceeds the threshold, Soft-NMS
        *decays* its confidence. This recovers detections of close/overlapping targets (e.g. ships
        sailing near each other) and requires no retraining.

        Decay rules:
            gaussian: s_i = s_i * exp(-iou² / sigma)
            linear:   s_i = s_i * (1 - iou)   if iou >= iou_threshold else s_i

        Args:
            boxes (torch.Tensor): Bounding boxes with shape (N, 4) in xyxy format (class-offset).
            scores (torch.Tensor): Confidence scores with shape (N,).
            iou_threshold (float): IoU threshold used by the linear method.
            sigma (float): Gaussian decay coefficient.
            method (str): "gaussian" or "linear".
            score_thres (float): Boxes whose decayed score falls below this are dropped.

        Returns:
            keep (torch.Tensor): Indices (into the input) of surviving boxes, ordered by decayed
                score (descending).
            scores_out (torch.Tensor): Per-box updated scores aligned to the original input indexing.

        Examples:
            >>> boxes = torch.tensor([[0, 0, 10, 10], [1, 1, 11, 11], [50, 50, 60, 60]], dtype=torch.float)
            >>> scores = torch.tensor([0.9, 0.85, 0.7])
            >>> keep, new_scores = TorchNMS.soft_nms(boxes, scores, 0.5)
        """
        device = boxes.device
        n = boxes.shape[0]
        scores_out = scores.clone()
        if n == 0:
            return torch.empty((0,), dtype=torch.int64, device=device), scores_out

        x1, y1, x2, y2 = boxes.unbind(1)
        areas = (x2 - x1) * (y2 - y1)
        idxs = torch.arange(n, device=device)  # remaining candidate indices (into original tensor)
        order = []  # picked indices in selection order

        while idxs.numel() > 0:
            # Select the current maximum-scoring box among the remaining candidates
            m = torch.argmax(scores_out[idxs])
            i = idxs[m]
            order.append(i)
            rest = torch.cat((idxs[:m], idxs[m + 1 :]))
            if rest.numel() == 0:
                break

            # IoU of the selected box with all remaining candidates
            xx1 = torch.maximum(x1[i], x1[rest])
            yy1 = torch.maximum(y1[i], y1[rest])
            xx2 = torch.minimum(x2[i], x2[rest])
            yy2 = torch.minimum(y2[i], y2[rest])
            ...
            # Decay the confidences of overlapping boxes
            if method == "linear":
                decay = torch.where(iou >= iou_threshold, 1.0 - iou, torch.ones_like(iou))
            else:  # gaussian
                decay = torch.exp(-(iou * iou) / sigma)
            scores_out[rest] = scores_out[rest] * decay

            # Drop candidates that fell below the score threshold (mirrors the original Soft-NMS prune)
            idxs = rest[scores_out[rest] > score_thres]

        keep = torch.stack(order) if order else torch.empty((0,), dtype=torch.int64, device=device)
        keep = keep[scores_out[keep] > score_thres]  # final threshold on selected boxes
        keep = keep[scores_out[keep].argsort(descending=True)]  # order by decayed score
