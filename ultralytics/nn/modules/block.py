...
class BiFPNAdd2(nn.Module):
    """Weighted fusion of 2 feature maps (BiFPN fast normalized fusion).

    Implements ``out = (w0 * x0 + w1 * x1) / (w0 + w1 + eps)`` with non-negative learnable weights
    (BiFPN, https://arxiv.org/abs/1911.09070). All inputs must share the same channel count and
    spatial size; the output keeps that channel count. Used at neck nodes that fuse an input feature
    with a single resized feature.
    """

    def __init__(self, eps: float = 1e-4):
        """Initialize with two learnable fusion weights.

        Args:
            eps (float): Small constant for numerical stability in the normalization.
        """
        super().__init__()
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        """Fuse two feature maps via fast normalized weighted addition."""
        w = torch.relu(self.w)
        w = w / (w.sum() + self.eps)
        return w[0] * x[0] + w[1] * x[1]


class BiFPNAdd3(nn.Module):
    """Weighted fusion of 3 feature maps (BiFPN fast normalized fusion).

    Implements ``out = (w0*x0 + w1*x1 + w2*x2) / (w0 + w1 + w2 + eps)`` with non-negative learnable
    weights. Used at neck output nodes that fuse the original input feature, the top-down feature and
    the bottom-up feature of the same level (the BiFPN skip connection).
    """

    def __init__(self, eps: float = 1e-4):
        """Initialize with three learnable fusion weights.
    ...
    def forward(self, x: list[torch.Tensor]) -> torch.Tensor:
        """Fuse three feature maps via fast normalized weighted addition."""
        w = torch.relu(self.w)
        w = w / (w.sum() + self.eps)
