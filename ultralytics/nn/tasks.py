...
def parse_model(d, ch, verbose=True):
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        elif m in frozenset({SESA, SENetV2, SimAM}):  # YOLO-SEA channel-preserving attention
            c1 = c2 = ch[f]
            args = [c1, *args]
        elif m in frozenset({BiFPNAdd2, BiFPNAdd3}):  # YOLO-SEA BiFPN weighted fusion (same-channel inputs)
            c2 = ch[f[0]]
        elif m in frozenset(
            {
                Detect,
